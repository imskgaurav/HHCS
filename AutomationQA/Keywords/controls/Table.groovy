...
public class Table extends BaseControl {
	...
	public void clickToCellLink(String rowIndex, String colName){
		...
		if(Utilities.tryToParseStringToInt(colName)) {
			cellLocator = String.format("%s//tr[%s]/td[%s]/a"
					,xpath,rowIndex.trim(),colName.trim());
		}else {
			cellLocator = String.format("%s//tr[%s]/*[count(%s/thead/tr/*[descendant-or-self::node()[starts-with(normalize-space(),'%s')]]/preceding-sibling::*)+1]/a"
	...
	public String getRowText(String rowIndex){
		String rowLocator = String.format("%s/tbody/tr[/*][%s]",xpath,rowIndex.trim());
	...
	public void waitForDataLoaded(int timeoutInSeconds){
		try{
			loadControl();
			String cellLocator = String.format("%s/tbody//tr[td]//td",xpath);
